...
function OpenedColor(text,ColourDisplay,Action,def)
	...
	if COLORPALLETE.Visible == true then
		ANIMATEFRAME01:TweenPosition(UDim2.new(-0.026, 0, -0.007, 0), 'Out', 'Quint', 0.25, true)
		wait(0.25)
		COLORPALLETE.BackgroundTransparency = 1
		Holder.Visible = false
		ANIMATEFRAME01:TweenPosition(UDim2.new(1.077, 0, -0.007, 0), 'Out', 'Quint', 0.25, true)
		wait(0.25)
		COLORPALLETE.Visible = false
	...
	SETCOLOR.MouseButton1Click:Connect(function()
		...
		pcall(function()
			Action(Color3.fromRGB(
				math.clamp(ColourDisplayBIG.ImageColor3.R * 255, 0, 255),
				math.clamp(ColourDisplayBIG.ImageColor3.G * 255, 0, 255),
				math.clamp(ColourDisplayBIG.ImageColor3.B * 255, 0, 255)
			))
		end)
	end)
...
UIStroke_MAIN.Color = Color3.fromRGB(91, 133, 197)
...
UIStroke_BG.Color = Color3.fromRGB(91, 133, 197)
UIStroke_BG.Thickness = 1
UIStroke_BG.Transparency = 0.25
UIStroke_BG.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
UIStroke_BG.Parent = BG
...
function library:AddWatermark(Text)
	...
	-- 圓角 + 描邊 + 陰影
	local uicorner = Instance.new("UICorner", obj1)
	uicorner.CornerRadius = UDim.new(0, 6)

	local stroke = Instance.new("UIStroke", obj1)
	stroke.Color = Color3.fromRGB(91, 133, 197)
	stroke.Thickness = 1
	...
	obj3.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
...
function library:Init(name)
	for _, v in pairs(Upper:GetChildren()) do
		if v:IsA('TextButton') then
			local isActive = v.Name == name
			local goalColor = isActive and Color3.fromRGB(210, 210, 210) or Color3.fromRGB(138, 138, 138)
			TweenService:Create(v, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
...
function library:AddWindow(text)
	...
	TEMPLATE_TEXT.BackgroundTransparency = 1
	TEMPLATE_TEXT.BorderSizePixel = 0
	TEMPLATE_TEXT.Position = UDim2.new(0, 0, 0.281, 0)
	TEMPLATE_TEXT.Size = UDim2.new(0, getsize(text), 0, 16)
	TEMPLATE_TEXT.ZIndex = 3
	TEMPLATE_TEXT.Font = Enum.Font.GothamSemibold
	TEMPLATE_TEXT.Text = text
	TEMPLATE_TEXT.TextColor3 = Color3.fromRGB(140, 140, 140)
	TEMPLATE_TEXT.TextSize = 16
	TEMPLATE_TEXT.AutoButtonColor = false

	local stroke = Instance.new("UIStroke", TEMPLATE_TEXT)
	stroke.Color = Color3.fromRGB(100, 100, 100)
	stroke.Thickness = 1
	stroke.Transparency = 0.8

	local uicorner = Instance.new("UICorner", TEMPLATE_TEXT)
	uicorner.CornerRadius = UDim.new(0, 4)

	TEMPLATE_TEXT.MouseEnter:Connect(function()
		TweenService:Create(TEMPLATE_TEXT, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(200, 200, 200)}):Play()
	end)
	TEMPLATE_TEXT.MouseLeave:Connect(function()
		TweenService:Create(TEMPLATE_TEXT, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(140, 140, 140)}):Play()
	end)
	...
	local scroll_corner = Instance.new("UICorner", HOLDER)
	...
	_LEFT.Position = UDim2.new(0.25, 0, 0.5, 0)
	...
	_RIGHT.Position = UDim2.new(0.75, 0, 0.5, 0)
	...
	TEMPLATE_TEXT.MouseButton1Click:Connect(function()
		...
		TweenService:Create(TEMPLATE_TEXT, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
	...
	local function getlarger(num)
		...
		if num == 1 then
			return (LeftSize >= RightSize) and "l" or "r"
		elseif num == 2 then
	...
	local function UpdateMainSize(f, anim)
		...
		local larger = getlarger(1)
		local targetSize = getlarger(2)[larger] + 15

		if anim then
			TweenService:Create(HOLDER, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
				CanvasSize = UDim2.fromOffset(0, targetSize)
			}):Play()
		else
			HOLDER.CanvasSize = UDim2.fromOffset(0, targetSize)
		end
	end
	...
	local function GetSide(typ, input)
		...
		elseif typ == 2 and input then
			if type(input) == "string" then
				local val = input:lower()
				if val == "left" or val == "l" then return _LEFT, LUIL end
				if val == "right" or val == "r" then return _RIGHT, RUIL end
			elseif input == 1 then
				return _LEFT, LUIL
			elseif input == 2 then
				return _RIGHT, RUIL
	...
	function AutoFit(section, list)
		if section and list then
			local x = section.AbsoluteSize.X
			local y = list.AbsoluteContentSize.Y
			section.Size = UDim2.new(0, x, 0, y + 8 + 23)
			UpdateMainSize()
		end
	end
